...
async def yesArgs(ctx, amount, criteria):
	...
	if not find.doesExist():
		...
		return
	embeds = getRandSauce(amount, find.getNumbers()) # get amount number of numbers given by criteria
...
def getRandSauce(amount, numbers):
	num = amount
	embeds = []
	if num > 0:
